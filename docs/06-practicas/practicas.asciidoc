...
*Vulnerabilidades*: _Broken Authentication_, falta de autenticación (Lack of Authentication), permitir acceso no autorizado (Allow Unauthorized Access), control de acceso deficiente (Broken Access Control) +
...
*Vulnerabilidades*: _Poor Data Validation_, problemas de conversión (Conversion Issues), desbordamiento aritmético (Arithmetic Overflow) +
...
==== _Implement appropriate Cross-Origin Resource Sharing policies_
...
==== Arquitectura integrada como _API gateway_
...
*Vulnerabilidad*: _Show the stack trace of the exception_ +
...
Una tarea para revisar y actualizar las configuraciones en toda la pila de API. La revisión debe incluir: archivos de orquestación, componentes de API y servicios en la nube [76].
...
*Vulnerabilidad*: Conexión insegura (unsecured connection), intercambio fraudulento de datos y fugas en la transmisión de datos (fraudulent data exchange and data transmission leaks). +
...
==== Monitorear todas las solicitudes y respuestas, (Monitor all the requests and responses, _Rate Limiting_)

*Vulnerabilidad*: Falta de limitación de tasa o no bloqueo de solicitudes maliciosas (Lack of rate limiting or not blocking malicious requests) +
Un interceptor dedicado en el algoritmo que supervisa todas las solicitudes y respuestas y, por lo tanto, previene ataques [75].
