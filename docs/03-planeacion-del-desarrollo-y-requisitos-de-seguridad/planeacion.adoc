...
| Fuga de datos sensibles hacia partes no autorizadas.
| Exponer datos personales en respuestas de API.
| Cifrado, control de acceso, minimización de datos.

| **D**enial of Service (Denegación de servicio)
| Ataques que impiden el funcionamiento legítimo del sistema.
| Saturación de peticiones en una API REST.
| Limitación de tasa (rate limiting), balanceo, detección de anomalías.

| **E**levation of Privilege (Elevación de privilegios)
| Un usuario gana permisos que no debería tener.
| Usuario regular que ejecuta operaciones administrativas.
| Separación de funciones, controles de acceso y validación de roles.
|===

====== Ejemplo práctico (según Green, 2022)

Green (2022) ilustra STRIDE con un ejemplo de un *portal de servicios web*, que puede adaptarse fácilmente al contexto de una API REST.  
Supongamos una API que gestiona información de clientes y sus pedidos.  
Al analizarla bajo STRIDE:

* **S – Spoofing:** Un atacante podría falsificar un token JWT para acceder como otro usuario.  
  *Mitigación:* emplear firmas HMAC o RSA y validación de expiración de tokens.  

* **T – Tampering:** Modificar el cuerpo JSON de una solicitud PUT para alterar el monto del pedido.  
  *Mitigación:* aplicar validaciones del lado servidor y controles de integridad sobre los datos.  

* **R – Repudiation:** Un usuario modifica información y luego niega haberlo hecho.  
  *Mitigación:* mantener auditorías firmadas digitalmente o logs inmutables.  

* **I – Information Disclosure:** La API responde con más información de la necesaria (por ejemplo, datos de tarjeta).  
  *Mitigación:* aplicar *output encoding* y filtrado estricto de respuestas.  

* **D – Denial of Service:** Se envían peticiones masivas para agotar recursos del servidor.  
  *Mitigación:* implementar *rate limiting*, *circuit breakers* y control de concurrencia.  

* **E – Elevation of Privilege:** Un usuario básico logra acceder a endpoints administrativos.  
  *Mitigación:* aplicar control de acceso basado en roles (RBAC) y validaciones de permisos en cada endpoint.

Este análisis evidencia cómo STRIDE guía la identificación de amenazas específicas y orienta las decisiones de diseño seguro dentro del ciclo de desarrollo de APIs (Green, 2022).
...
Green, J. (2022). *Security Architecture*. BCS, The Chartered Institute for IT.  
Threat Modeling Manifesto. (2020). *Threat Modeling Manifesto*. Recuperado de https://www.threatmodelingmanifesto.org  
OWASP. (s.f.). *OWASP Threat Modeling*. Recuperado de https://owasp.org/www-community/Threat_Modeling
