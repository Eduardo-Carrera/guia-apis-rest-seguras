...
Estándar industrial adoptado para transmitir información de manera segura entre partes mediante un formato compacto y verificable. Facilita incluir datos de autenticación y autorización en las solicitudes y respuestas de la API, sin requerir almacenamiento adicional en el servidor. En este modelo, el cliente envía un token firmado en el encabezado de autorización HTTP, y el servidor verifica el token para extraer los datos de autenticación y autorización del usuario (EP-03).
