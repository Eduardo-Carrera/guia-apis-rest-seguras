...
La Especificación de Requisitos de Software (SRS) también puede denominarse especificación funcional, especificación del producto, documento de requisitos o especificación del sistema, aunque estos términos no siempre se emplean de manera uniforme en todas las organizaciones [22].
...
* Los clientes, el área de mercadotecnia (marketing) y el personal de ventas lo utilizan para conocer el producto que se espera entregar.  
...
Estas prácticas organizacionales y modelos, como confianza cero (Zero Trust), resultan especialmente relevantes en el diseño y operación de APIs REST, dado que la exposición directa a internet incrementa la superficie de ataque y exige la alineación entre los aspectos técnicos y humanos dentro de una misma estrategia de seguridad.
...
En este token, las declaraciones (claims) incluidas son `iss`, `sub`, `aud`, `exp`, `nbf`, `iat` y `jti`. Estas son declaraciones (claims) reservadas definidas en el RFC de JWT. Aunque no son obligatorias, proporcionan un conjunto mínimo de información útil para la autenticación y autorización.
...
* Permite control granular de los derechos de acceso mediante las declaraciones (claims) del token.  
...
OIDC amplía el servidor de autorización OAuth 2.0 para que actúe también como proveedor de identidad, entregando al cliente un *ID token* (un JWT con claims sobre el usuario) cuando se utiliza el alcance (scope) especial `openid`. Además, se pueden incluir alcances (scopes) adicionales como `profile`, `email`, `address` o `phone`, permitiendo obtener información más completa sobre el usuario [33].

Los flujos definidos por OIDC son tres: flujo del código de autorización (Authorization Code Flow), flujo implícito (Implicit Flow) y flujo híbrido (Hybrid Flow). El recomendado es el flujo del código de autorización (Authorization Code Flow) junto con PKCE, ya que ofrece mayor seguridad.
...
Aunque SAML no está diseñado para usarse directamente en APIs REST, existe una extensión llamada Perfil de SAML 2.0 para la Autenticación de Clientes y la Concesión de Autorización en OAuth 2.0 (*SAML 2.0 Profile for OAuth 2.0 Client Authentication and Authorization Grants*), la cual permite a un cliente solicitar un token de acceso (*access token*) usando SAML, siempre que el servidor de autorización tenga soporte para esta funcionalidad [33].  
...
El Intercambio de recursos de origen cruzado (Cross-Origin Resource Sharing, CORS) es un estándar que permite que aplicaciones web realicen solicitudes a APIs alojadas en dominios distintos al de la aplicación cliente. Este mecanismo surge como extensión controlada de la política del mismo origen (Same-Origin Policy), la cual bloquea solicitudes entre dominios diferentes para proteger contra ataques como Falsificación de solicitudes entre sitios (Cross-Site Request Forgery, CSRF) (CSRF). [37]  
...
Este estándar permite configurar políticas de CORS a nivel global para toda la aplicación o de manera selectiva por el punto de acceso (endpoint). Las políticas pueden definir qué dominios, métodos y cabeceras están permitidos, manteniendo la seguridad de la política del mismo origen al tiempo que habilitan la interoperabilidad entre aplicaciones y servicios en distintos dominios.  
...
El documento propone un modelo de protección integral del ciclo de vida de las APIs, basado en los principios de DevSecOps y el paradigma de confianza cero (Zero Trust), considerando controles tanto previos a la ejecución (pre-runtime) como durante la ejecución (runtime). 
...
* **Vectores de ataque:** Rutas o métodos empleados por un atacante para obtener acceso o explotar una vulnerabilidad, como correos de suplantación de identidad (phishing), puertos abiertos o bibliotecas comprometidas.  
...
| Suplantación de identidad (**S**poofing)
...
| Manipulación de datos (**T**ampering)
...
| Divulgación de información (**I**nformation Disclosure)
...
| Denegación de servicio (**D**enial of Service)
...
|Elevación de privilegios (**E**levation of Privilege)
...
| DE-5 | El servidor web que aloja la puerta de enlace (API Gateway) se encuentra detrás de un cortafuegos (firewall), y toda comunicación externa está restringida únicamente a conexiones TLS (HTTPS).
|===
...
* Identificador (ID): Un identificador único asignado al punto de entrada. Este se utilizará para hacer referencia cruzada entre el punto de entrada y cualquier amenaza o vulnerabilidad identificada. En el caso de puntos de entrada organizados en capas, se debe usar una notación mayor.menor.
...
* Identificador (ID): Se asigna un identificador único a cada activo. Este identificador se usa para hacer referencias cruzadas con las amenazas o vulnerabilidades identificadas.
...
* Identificador (ID): Se asigna un número único a cada nivel de confianza. Este se utiliza para hacer referencia cruzada con los puntos de entrada y los activos.
...
* Suplantación de identidad (**Spoofing**) aplicar autenticación fuerte, MFA y validación de tokens.  
* Manipulación de datos (**Tampering:**) cifrar datos en tránsito y validar integridad.  
* Repudio (**Repudiation:**) habilitar registro de auditoría con trazabilidad.  
* Divulgación de información (**Information Disclosure:**) implementar controles de acceso y clasificación de datos.  
* Denegación de información (**Denial of Service:**) limitar la tasa de peticiones y usar sistemas de detección de anomalías.  
* Elevación de privilegios (**Elevation of Privilege:**) revisar roles, permisos y aplicar el principio de mínimo privilegio.
